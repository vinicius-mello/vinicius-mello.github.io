...
preprocessSource = (filename, source, sourceID, callback) ->
  ...
  for line in source.split /\r?\n/
    if match = line.match /^\s*#include\s+"(.*?)"\s*$/
      ...
      shadersToInclude++
      do (insertLocation) ->
        loadShaderSource cofgl.joinFilename(filename, match[1]), (source) ->
...
class Shader extends cofgl.ContextObject
  @withStack 'shader'
  ...
  getUniformLocation: (name) ->
    @uniformCache[name] ?= cofgl.engine.gl.getUniformLocation @prog, name

  getAttribLocation: (name) ->
    @attribCache[name] ?= cofgl.engine.gl.getAttribLocation @prog, name

  uniform1i: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform1i loc, value if loc

  uniform1f: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform1f loc, value if loc

  uniform2f: (name, value1, value2) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform2f loc, value1, value2 if loc

  uniform2fv: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform2fv loc, value if loc

  uniform3fv: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform3fv loc, value if loc

  uniform4fv: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniform4fv loc, value if loc

  uniformMatrix3fv: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniformMatrix3fv loc, false, value if loc

  uniformMatrix4fv: (name, value) ->
    loc = this.getUniformLocation name
    cofgl.engine.gl.uniformMatrix4fv loc, false, value if loc

  bind: ->
    cofgl.engine.gl.useProgram @prog

  unbind: ->
    cofgl.engine.gl.useProgram null
